...
function getWeekNumberByDate(date) {
  ...
  const firstThursday = new Date(Date.UTC(tempDate.getFullYear(), 0, 4));
  const diffDays = Math.floor((tempDate - firstThursday) / 86400000);
  return Math.ceil((diffDays + 4) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  while (true) {
    ...
    if (next13th > date && next13th.getDay() === 5) {
      return next13th;
    }
    currentMonth += 1;
    if (currentMonth > 11) {
